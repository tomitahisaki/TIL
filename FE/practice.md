...
## TLS通信
必須のサーバ認証とは別にオプションでクライアント認証を行うこともできる

PCとWebサーバは、利用者個人のデジタル証明書に付された認証局の署名を検証することで、デジタル証明書の正当性を確認します

デジタル証明書が正当なものであれば、利用者の真正性が証明される
...
- 分岐網羅: 全ての分岐を少なくとも1回は実行するテストケースを設計する

分岐網羅をすべて満足するなら、命令網羅も満足する。ただし、逆は成立しない

## 条件網羅
判定条件が複数あるときに、それぞれの条件が真・偽の場合を組合せたケースを設計する

xとyの場合は、xがtrue・yがfalse・xがtrue・yがfalseという条件を含む必要がある。
...
TCP/IP・HTTP・SMTPなどをプロトコルを標準化している
...
## RAID
RAID0: ストライピング 複数のディスクに分散してデータを書き込む

RAID1: ミラーリング 2つのディスク同時に書き込み冗長化する
...
## エンタープライズアーキテクチャ(EA)
ビジネス・データ・アプリケーション・テクノロジの4つに分かれている

アプリケーションは、業務プロセスを支援するシステムの機能や構成などを体系的に示したもの
