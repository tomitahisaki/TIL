...
## ファイル管理
ディレクトリを用いて管理する(WindowsやMacでいうフォルダ)

ファイルを効率よく管理するために階層構造となっている 階層構造の最上位にあるディレクトリを**ルートディレクトリ**と呼ぶ 

**サブディレクトリ**: ディレクトリの下位に作成されたディレクトリ

**カレントディレクトリ**: 現在、操作対象にあるディレクトリ

### パス指定
**絶対パス**:  ルートディレクトリを基点に、目的となるディレクトリやファイルまでのファイルを指定する( /next/child/page.html のような指定)

**相対パス**: カレントディレクトリを基点に、目的となるディレクトリやファイルまでの経路を指定する( ./child/page.html のような指定)

1階層上の場合は、`../`で表す

### データのバックアップ
- フルバックアップ: 磁気ディスクに保存されるすべてのデータをバックアップする
- 差分バックアップ: 前回のフルバックアップ以降に変更されたデータをバックアップする
- 増分バックアップ: 前回のバックアップ以降に変更されたデータをバックアップする

フルバックアップは、バックアップの時間は長くなるが、復元時間が短く済む

差分や増分バックアップの場合、フルバックアップに、それぞれのバックアップを反映させる必要があるため、復元時間が長くなる
...
#### 静止画
- BMP: Windows標準 圧縮されない
- GIF: 256色 可逆圧縮 イラストやアイコンに使われる
- PNG: 256色 フルカラー(PNG-24)の2種類 可逆圧縮 部分的に透明にできる Web画像などに使われる
- JPEG: 国際標準規格 フルカラー 不可逆圧縮 写真などに使われる
#### 動画
- MPEG: 国際標準規格 不可逆圧縮 
  - MPEG-1: ビデオCD用
  - MPEG-2: DVDやデジタル放送用の企画
  - MPEG-4: MPEG-2より圧縮効率が高く、ビデオやワンセグ放送で使われる。**H.264/MPEG-4AVC**として規格化されている MP3はMPEG1の音声部分の圧縮アルゴリズムを使用した音声データの圧縮方式


**画像データ**
- ラスタデータ: 写真などのようにピクセルの集まりで画像を表現する
- ベクタデータ: 座標のいちや線分の長さなどを演算で表現する

**SVG**: 円や直線、文字列などの図形オブジェクトをXMLで記述し、Webページでの図形表がにも使うことができる画像フォーマット

#### マルチメディアの応用
**CG**: コンピュータを使って画像を処理・生成する技術、またはその画像

**VR**: バーチャルリアリティ 仮想空間に入り込んだような効果を生み出す技術

**AR**: 目の前にある現実の情景や風景の映像をコンピュータに取り込み、現実には存在しない仮想の情報を合瀬いして映し出す技術
