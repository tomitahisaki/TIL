...
### PWM
モータの回転速度やLEDの明るさ等をデジタル信号で制御する方式
...
- 有限オートマトン: 初期状態からいくつかの状態を遷移し、最終的に受理状態(終了状態)になるもの
- 状態遷移図: オートマトンの状態の遷移を図にしたもの
- 状態遷移表: オートマトンの状態遷移を表にしたもの

# AI
人が行うような学習や認識、予測、判断等の知的な活動をコンピュータにさせる取り組みやその技術のこと

## 機械学習
大量のデータ(ビックデータ)をコンピュータに解析させ、コンピュータ自らが予測や判断できるように学習させること

- 教師あり学習: 予め問題と正解をコンピュータに提示し、誤りを指摘したりすることで、コンピュータ自らがそれら特徴を学習すること
- 教師なし学習: コンピュータ自ら、統計的性質やある種の条件に従い、データのグループ分けや情報の集約を行う
- 強化学習: 試行錯誤を通して、報酬が最も多く得られるような方策を学習すること

## ディープラーニング
人の脳神経回路を模倣したモデル(ニューラルネットワーク)で解析し、AI自らがデータを判別するための特徴を探し出すこと

- 超スマート社会: サイバー空間と現実空間が高度に融合した社会
- Society5.0: 超スマート社会の進展により、年齢・性別・言語に違いに関わらず、快適に生活できる社会。狩猟・農耕・工業・情報という社会に続く、5番目の社会と呼ばれている
- デジタルツイン: さまざまなセンサから収集したデータを用いて、デジタル空間に現実世界のような世界を作り上げ、現実世界では実施できないようなシミュレーションを行う

# 線形代数
- スカラー: 大きさを表す数値
- ベクトル: 数値、スカラーを1列に並べたもの
  - 成分: ベクトルを構成する要素
...
### 行列のスカラー倍
行列を2倍、1/2倍にすること。各成分それぞれに掛ける

### 行列同士の乗算
行列Aと行列Bの乗算の場合、行列Aの列数と行列Bの行数が同じ必要がある。

m行n列の行列、n行p列の行列しか乗算できない。結果は、m行p列の行列となる

### 逆行列
- 正方行列: 行と列が同じ数になっている行列
- 単位行列: 左上からの対角線上の成分が1、それ以外の成分が0の正方行列
...
## 組合せ
n個の中から並び順を考慮せずにr個取り出せたもの

## 統計
- 平均値: 各データの合計をデータの個数で割って求めた値
- 中央値(メジアン): データを順番に並べて、中央に位置する値
- 最頻値(モード): 出現頻度の最も高い値
- 範囲(レンジ): データの最大値と最小値
- 分散: 平均値からばらつきを表し、偏差(平均値との差)の2乗の総和の平均値
- 標準偏差: 分散の平方根 偏差が小さいと、ばらつきが少ない。偏差が大きいと、ばらつきが大きい
