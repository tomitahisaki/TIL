...
- SEIM アクセスログから、Webサイトの脆弱性を悪用した攻撃を検出するツール
...
- 排他制御 主記憶へのアクセス高速化のため、アクセス要求、データの読み書き及び後処理が終わってから、次のメモリアクセス処理にうつる
...
クライアントからサーバに対して、ストアドプロシージャ1回の要求で、データベースに複数のクエリを発行できるので、直に同数のクエリを発行する場合と比較して、ネットワーク負荷を軽減できる
...
- PNG 可逆圧縮の画像ファイルフォーマット
- TIFF 異なる形式で一つのファイルに格納できる形式
...
- 表・ビュー・インデックス・他オブジェクト
- 参照・検査制約、ユーザー情報、アクセス権
...
- ステルス型マルウェア: ルートキットを利用して、マルウェアに感染していないように見せかけることによって、マルウェアを隠蔽する
...
- 位置
- 移動 表が別のサーバに異動されても、プログラムは表が配置されたサーバを意識する必要がない
- 分割 表が複数のサーバに分割されて配置されても、プログラムは分割された配置を意識する必要がない
- 複製 複数のサーバに1つの表が重複していても、プログラムは表の重複を意識する必要がない
- 障害
- データモデル サーバのDBMSが異種であっても、プログラムはDBMSの相違を意識する必要がない
...
- 非対称型マルチプロセッサシステム それぞれの役割が決定されている複数のプロセッサによって処理を分散すう方式
- 対象型マルチプロセッサシステム カーネル・ユーザプロセスを区別せずに同等な複数のプロセッサに処理を分散する方式
...
- トラッキング ウェアラブルや完成センサーなどを用いて非言語情報を認識する処理
- レンダリング 仮想世界の情報をディスプレイに描画可能な形式の画像に変換する処理
- ビューイング変換 現実と仮想世界の融合させるために、それぞれの世界に定義した3次元座標を一致させる処理
- モデリング変換 時間経過とともに生じる者の移動などの変化について、モデル化したものを物理法則などに当てはめて変化させる処理
...
サブルーチン呼び出し時に、戻り先アドレス・レジスタの内容などを格納するメモリのアクセス
...
- NTP Network Time Protocol: 123番を使う
...
ハードウェア上で直接稼働し仮想マシンの土台となるミドルウェア

- シストリックアレイ 多数のプロセッサとバッファの組をマトリクス上に結合して構築される並列計算機のアーキテクチャ
- デスクトップグリッド 家庭内のPCやオフィスのコンピュータのインターネットに接続された計算機を1つの仮想的名コンピュータシステムとして結びつけ、大規模な計算を行うこと
- モノリシックカーネル カーネル自体に多機能をもたせるアーキテクチャ

## サーバプロビジョニングツール

システム構成をあらかじめ記述することで、サーバを自動的に構成する
