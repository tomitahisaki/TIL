...
- Standard-IA：infrequency Access の略。低頻度アクセスデータ用のストレージ。One Zone より重要なマスターデータ向け。データ取得は早い。standard と one zone の中間の価格
...
概念実証、本番稼働の HPC コンピューティング環境を迅速に構築して、デプロイできる

AWS Batch や、Slurm スケジューラとともに使用可能

## EFA

Message Passing Interface(MPI)を使用する HPC アプリケーション、NVIDIA Collective Communications Library(NCCL)を使用する Machine Learning(ML)をスケールできる

## サードパーティのアプリケーションを EC2 からの API 経由でアクセスしたい場合

外部 Web アプリケーションに対して、EC2 インスタンスと連携するための外部アクセス設定が必要

アプリケーションの提供ベンダーのみ利用するように制限が必要

回答

外部 Web アプリケーションが必要とする権限を設定した IAM ポリシーを作成

クロスアカウントアクセスが可能となる IAM ロールを作成し、外部 Web アプリケーションに設定
