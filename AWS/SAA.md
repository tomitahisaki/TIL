...
EC2 の場合は、インスタンスのステータスチェックの結果を確認する
...
- 動的スケーリング
  - 簡易スケーリングポリシー：ターゲット追跡スケーリングポリシーの通常設定。アラーム設定に基づいて 1 段階目のスケーリングを実施 CPU 利用率をモニタリングする
...
ヘルスチェックに応答がないインスタンスは負荷分散先から除外されるが、EC2 はステータスチェックで正常であれば、インスタンスは起動状態が続く。インスタンスが終了しないと、新規起動しないので、最小キャパシティ未満でのインスタンスで負荷分散する
...
- Desired Capacity：Auto Scaling が実行されない状態でインスタンス数を設定する。数値を変更することで、手動でスケーリング可能
- Minimum Capacity：スケールイン時にインスタンスを削減する際の下限のインスタンス数を設定する。希望する容量より大きい数値は設定できない
- Maximum Capacity：スケールアウト時に起動するインスタンスの最大数を設定できる。希望する容量より少ない数値を設定できない
...
定期的にヘルスチェックを行い、特定の EC2 へのルーティング停止する。負荷分散などが可能
...
そのため、インバウンドとアウトバウンドのどちらも設定する必要がある

インバウンドだけだと、リクエストを受け付けても、レスポンスを送信しないので正常に通信できない
...
ステートフルなので、インバウンド設定をすれば、通信が許可される。
...
インターネットから VPC 内の EC2 インスタンスへアクセスできる
...
仮想化プライベートゲートウェイ：VPN トンネルの Amazon 側にあるルーター

## NAT ゲートウェイ

NAT ゲートウェイは、プライベートサブネットからインターネットへの通信を可能にする機能

IPv4 専用の機能。AWS によって AZ 内で冗長化されている
...
プライベートサブネットのルートテーブルに NAT ゲートウェイへのルーティングを設定すること
...
スポットフリート：スポットインスタンスのオプション。必要なインスタンス数を指定することで、指定した数のスポットインスタンスが起動する

### タグ

タグを利用することで、インスタンス、イメージ、およびその他の Amazon EC2 リソースを分類するためのメタデータを各リソースに割り当てることができます。 タグは目的、所有者、環境など、さまざまな方法で AWS リソースを分類できます
...
- インスタンスストア: 揮発性のブロックストレージ（インスタンス稼働中にだけ利用可能）
- インスタンスファミリー:
...
- Intelligent-Tiering: ミリ秒単位のアクセスが可能で、アクセス頻度の高いデータ。アクセス頻度が予測できないデータに適している
- Standard-IA：infrequency Access の略。低頻度アクセスデータ用のストレージ。One Zone より重要なマスターデータ向け。データ取得は早い。Standard と One Zone-IA の中間の価格。アクセス頻度が高いデータには適していない。
- One Zone-IA：低頻度アクセス用のストレージ。マルチ AZ 分散されていないので可用性が低い。重要ではないデータ向け。データ消失リスクがある。
...
S3 Glacier は、Advanced Encryption Standard(AES-256)を使用して、サーバーサイド暗号化が自動的に適用される。
...
静的サイトを構築する際に、S3 に構築可能

静的な Web サイトを構築するには、静的ウェブホスティング機能を有効にする

index.html を設定し、Web サイトの設定を完了。

インターネットからアクセス可能にするようにパブリックアクセスブロックを無効化にする。バケットポリシーの設定によってインターネットからバケットへの S3.GetObject を許可する設定が必要

HTML オブジェクトにアクセスする際は、オブジェクトの読み取りアクセス許可を付与する必要がある。

アクセス許可がないと、403 のレスポンスコードを返す
...
- 大容量取り出し：標準取り出しより時間がかかるが、取り出し料金が低価格になる。所要時間：5~12 時間。１日以上でも問題なければ、こちらがコスパが良い
...
- ID ベースのポリシー：IAM ユーザー、IAM グループ、IAM ロールにアタッチするポリシー
  - 管理ポリシー
    ...
    - カスタマー管理ポリシー：ユーザーが定義するポリシー、世代管理可能。AWS 管理ポリシーで必要な権限を付与できない場合に使用
  - インラインポリシー：ユーザーが対象毎に作成・付与するポリシー
- リソースベースのポリシー：AWS リソースにアタッチするポリシー。特定のユーザーや特定の IP アドレスだけに許可する場合に利用
...
Lambda 関数から DynamoDB へのアクセスしてデータ参照したいときは、Lambda 関数に IAM ロールを割り当てる

IAM グループ：複数の IAM ユーザーが所属するグループ。同一アカウント内の複数の IAM ユーザーのアクセス権限を一元的に管理できる
...
トリガーを設定して、実行できる。リクエストが不定期に発生し、1 リクエストが数秒で終わる処理に適している リアルタイム処理には適していないので注意
...
- プログラム実行環境の構築や運用が必要ない
- プログラムはリージョン内の複数の AZ に分散配置される(耐障害性・可用性の向上)
- プログラムへのリクエスト数に応じたスケーリングが自動で行われる(拡張性の向上)
...
- インターネットへアクセスできない
- 指定したサブネットへのアクセスが可能になる（パブリック・プライベートゲートどちらも）

その上で、NAT ゲートウェイを使用してパブリックサブネット経由でインターネットに接続可能

VPC アクセス設定が無効：

- パブリックなリソースはインターネット経由でのアクセス可能
- プライベートサブネットへのアクセス不可能
...
- クラスター：1 つ以上のタスクまたはサービスに構成される論理グループ。コンテナが動作する VPC やサブネット
- タスク：実行環境の CPU やメモリのスペック、実行するコンテナイメージ、コンテナから他 AWS サービスへのアクセス権限。
...
- HTTP
  - 使用した分の API コールだけ料金が発生
- WebSocket
  - 送受信したメッセージ数および分単位の接続合計時間から料金が発生
- RESTful
  - 受信した API コールと、転送したデータ量に対してのみ料金が発生する
...
- Snowball Edge Storage Optimized：ストレージ利用が主目的。ローカルストレージや大規模データ転送に適している
  - ブロックボリュームと S3 互換オブジェクトストレージに 100TB の HDD 容量を提供する
- Snowball Edge Compute Optimized：エッジコンピューティング向けサービス。vCPU やメモリが潤沢。ネットワークに接続していなくても高度な情報処理を行う場合に適している
  - S3 互換オブジェクトストレージや EBS 互換ブロックボリュームように 42TB の HDD 容量を提供する
...
実行中のインスタンスからボリュームをデタッチする必要がある

→ 古いインスタンスを停止し、ボリュームをデタッチする必要がある。
...
シングルスターレプリケーションを使用して、DB クラスターに対する急激な接続やワークロードの増加を処理できる。

負荷が減少した場合は、未使用のプロビジョニングされた DB インスタンスに対して料金が発生しないように、不要な Aurora レプリカを削除する
...
## Aurora for MySQL と RDS for MySQL

Aurora for MySQL は、標準的な MySQL データベースに比べて、最大で 5 倍、標準的な PostgreSQL データベースに比べて、最大で 3 倍高速化することが可能なリレーショナルデータベース。

Aurora for MySQL のほうが、高可用性・スケーラビリティを高めることが可能
...
- Web セッション管理：Time-to-Live メカニズムにより、セッション管理を簡単に管理可能
- メタデータ保存：主キーでインデックスをつけられたデータを格納することができる。1Byte から最大 400KB までの葉にへの低レイテンシーの読み取りおよび書き込みアクセス可能。
...
YAML を使用して、サーバレスアプリケーションの Lambda 関数、API、データベース、イベントソースマッピングをモデリングすることができます。AWS SAM がアプリケーションをデプロイする際は CloudFormation と連携します。その際は、SAM が SAM 構文を AWS CloudFormation 構文に変換および拡張することで、CloudFormation テンプレートからサーバーレスアプリケーションの構築を実行します
...
IPv4 と IPv6 の両方のホストゾーンに 4 つのリゾルバー IP アドレスの一意のセットを提供します。各 IP アドレスは、Route 53 ロケーションの異なるサブセットに対応します。各ロケーションサブセットは、他のサブセットのインフラストラクチャと部分的にしか重複しない権限を持つ DNS サーバーで構成されます。これにより、何らかの理由でユーザークエリが失敗した場合、再試行時に正常に処理されるようになります

### エニーキャストルーティング

ネットワークの近接度に基づいて DNS クエリを最も近いエッジロケーションに送信します。エッジロケーションのグローバルに分散されたネットワークから動作させることで、ユーザーによるアプリケーションへのアクセスが可能なように支援します
...
ビックデータの処理や分析を行うサービス リアルタイムでデータ解析可能

ビッグデータを処理する既存のフレームワークである Hadoop や Spark を用いている。

データを複数配置して並列に処理する「分散処理」により、迅速かつ効率のよい処理を実現している

Amazon EMR の中心的なコンポーネントはクラスターです。このクラスターとは、Amazon Elastic Compute Cloud (Amazon EC2) インスタンスの集合です。クラスター内の各インスタンスはノードと呼ばれます。Amazon EMR は、各ノードタイプにさまざまなソフトウェアコンポーネントをインストールすることで、Apache Hadoop などの分散型アプリケーションを構成することができます

### Apache Spark Streaming

Kinesis Data Streams からストリーミングされるイベントをリアルタイムに分析できるストリーミングデータ処理パイプラインを構築できる

Auto Scaling を有効にすることで、負荷に応じて処理能力を向上できる
...
Microsoft Active Directory と互換性のあるフルマネージド型の AD を作成

AWS とオンプレミスの Microsoft AD の間で信頼関係を設定、SSO と連携したシングルサインオンが可能
...
最大 500~5000 ユーザーを管理。スモールとラージによる

## AD Connector

Active Directory と IAM を統合

Microsoft Active Directory へリダイレクトするのに使用するディレクトリゲートウェイ

既存の会社の認証情報を使用して、WorkSpaces や WorkDocs などの WorkMail アプリケーションにログオン可能になる

SSO と連携したシングルサインオンが利用可能
...
インスタンスは、クライアントと Web サーバー間の接続時にはセッションがアクティブである限り、データベースが使用されたままになるので、分散システムに適さず、サーバー・データーベースの負荷も増加する

ステートレスの場合、

インスタンスは、状態を維持しないので、分散処理に利用することが可能。スケールイン/スケールアウトが容易に実現可能
